    ...
    def add_bit(self, name, bitpos, reset, hw, sw):
        ...
        if name != '':
            ...
            if self.cur_field != None:
                ...
                if self.cur_field.name in [x.name for x in self.fields]:
...
#        self.invertedmask = (~self.mask)&0xFFFFFFFF
        ...
        for field in self.fields:
            ...
            if field.sw in sw_writable:
                ...
                if len([x for x in self.fields if x.sw in sw_writable]):
        ...
        if self.sw in sw_writable:
            self.writable = True
            self.writable_fields = [x for x in self.fields if x.writable]
        else:
            self.writable = False
...
class xls_block:
    ...
    def end_register(self):
        ...
        if self.cur_reg.addr in [x.addr for x in self.registers]:
        ...
        if self.cur_reg.name != "NOREGISTER":
            if self.cur_reg.name in [x.name for x in self.registers]:
            ...
            # name translation for MAC registers
            if self.cur_reg.name in trans_reg:
    ...
    def end(self):
        ...
        if len(self.registers) == 0:
            ...
            highest = max([x.addr for x in self.registers])
...
class xls:
    ...
    def print_all(self):
        ...
        for i in range(self.wsheet.nrows):
            for j in range(self.wsheet.ncols):
